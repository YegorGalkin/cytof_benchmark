...
from configs.pbt.wae_pbt import get_config
...
def vae_train(cfg):
    ...
    while True:
        ...
        MSE, MMD, loss = test(model, X_val_batches)
...
if __name__ == '__main__':
    ...
    def explore(config):
        config["batch_size"] = min(config["batch_size"], 4*1024)
        return config
    ...
    scheduler = PopulationBasedTraining(
        ...
        hyperparam_mutations={
            ...
            "batch_size": tune.qlograndint(256, 8 * 1024, 32, base=2),
        },
    ...
    tuner = tune.Tuner(
        ...
        run_config=air.RunConfig(
            local_dir='/data/PycharmProjects/cytof_benchmark/logs/ray_tune/WAE_MMD',
        ...
        param_space={
            ...
            "learning_rate": tune.loguniform(1e-9, 1e-1),
            "batch_size": tune.qlograndint(256, 4 * 1024, 32, base=2),
    ...
    torch.save(best_result.checkpoint.to_dict(), '/data/PycharmProjects/cytof_benchmark/logs/ray_tune/WAE_MMD/WAE.pth')
    with open('/data/PycharmProjects/cytof_benchmark/logs/ray_tune/WAE_MMD/WAE_MMD_summary.txt', 'w') as f:
        print('val_mse,{}'.format(best_result.metrics['MSE']), file=f)
        print('val_mmd,{}'.format(best_result.metrics['MMD']), file=f)
