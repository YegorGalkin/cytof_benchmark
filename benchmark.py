...
if __name__ == '__main__':
    ...
    for model, config, latent in zip(models, configs, latents):
        for dataset, data_dir, in_features, interval in zip(datasets, data_dirs, features, intervals):
            os.system(f'python3 pbt_runner.py '
                      ...
                      f'--config.latent_dim={latent} '
                      f'--config.perturbation_interval={interval} '
                      f'--config.local_dir={os.path.join("/home/egor/Desktop/ray_tune/pbt_bench/dim5", model, dataset)} '
